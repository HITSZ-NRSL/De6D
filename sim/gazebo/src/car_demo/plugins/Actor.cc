    ...
    : dataPtr(new PriusHybridPluginPrivate)
{
  int argc = 0;
  char *argv = nullptr;
  ros::init(argc, &argv, "ActorPlugin");
...
{
  gzwarn << "ActorPlugin loading params" << std::endl;
  gzwarn << "hi kostas" << std::endl;
  ...
  gzwarn << this->robot_namespace_ << std::endl;
  ros::NodeHandle nh(this->robot_namespace_);
  std::string topic = "prius";
  if (_sdf->HasElement("controlTopic"))
    //topic = _sdf->GetElement("controlTopic");
    topic = _sdf->Get<std::string>("controlTopic");
  gzwarn<<topic<<std::endl;
  this->dataPtr->controlSub = nh.subscribe(topic, 10, &PriusHybridPlugin::OnPriusCommand, this);
  ...
  gzwarn << dPtr->model->GetName() << std::endl;
