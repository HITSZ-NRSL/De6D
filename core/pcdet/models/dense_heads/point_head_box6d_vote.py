...
class PointHeadBox6DVote(PointHeadTemplate):
    """
    ...
    def assign_stack_targets_simple(self, points, gt_boxes, extend_gt_boxes=None, set_ignore_flag=True):
        ...
        for k in range(batch_size):
            ...
            # vote point cls label
            point_cls_labels_single = point_cls_labels.new_zeros(bs_mask.sum())
            box_idxs_of_pts = box_utils.points_in_boxes3d(
                points_single,
                gt_boxes[k:k + 1, :, 0:9].squeeze(dim=0)
            ).long()
            box_fg_flag = (box_idxs_of_pts >= 0)
            point_cls_labels_single[box_fg_flag] = 1

            if extend_gt_boxes is not None:
                extend_box_idx_of_pts = box_utils.points_in_boxes3d(
                    points_single,
                    extend_gt_boxes[k:k + 1, :, 0:9].squeeze(dim=0)
                ).long().squeeze(dim=0)
                fg_flag = box_fg_flag
            ...
            point_reg_labels_single[box_fg_flag] = gt_box_of_fg_points[:, 0:3]  # vote to boxes center
    ...
    def assign_targets_simple(self, points, gt_boxes, extra_width=None, set_ignore_flag=True):
        ...
        ).view(batch_size, -1, gt_boxes.shape[-1]) if extra_width is not None else gt_boxes
    ...
    def assign_stack_targets_mask(self, points, gt_boxes, extend_gt_boxes=None,
        ...
        for k in range(batch_size):
            ...
            point_cls_labels_single = point_cls_labels.new_zeros(bs_mask.sum())
            box_idxs_of_pts = box_utils.points_in_boxes3d(
                points_single, gt_boxes[k:k + 1, :, 0:9].squeeze(dim=0)
            ).long()  # (N)
            box_fg_flag = (box_idxs_of_pts >= 0)
            if set_ignore_flag:
                extend_box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(
                    points_single, extend_gt_boxes[k:k + 1, :, 0:9].squeeze(dim=0)
                ).long()
                fg_flag = box_fg_flag
            ...
            if gt_box_of_fg_points.shape[0] > 0:
                ...
                point_reg_labels_single[fg_flag] = fg_point_box_labels[..., :self.box_coder.code_size]
                point_reg_labels[bs_mask] = point_reg_labels_single
    ...
    def get_axis_aligned_iou_loss_lidar(self, pred_boxes: torch.Tensor, gt_boxes: torch.Tensor):
        ...
        vol_p = len_p.prod(dim=-1)  # 预测体积
        vol_g = len_g.prod(dim=-1)  # 实际体积
    ...
    def get_corner_loss_lidar(self, pred_boxes: torch.Tensor, gt_boxes: torch.Tensor):
        ...
        if pred_boxes.shape[1] > 7:
            pred_box_corners = box_utils.boxes3d_to_corners_3d(pred_boxes)
            gt_box_corners = box_utils.boxes3d_to_corners_3d(gt_boxes)
        else:
    ...
    def get_box_layer_loss(self, tb_dict=None):
        ...
        if isinstance(self.box_coder, box_coder_utils.PointBinResidualCoder):
        ...
        elif isinstance(self.box_coder, box_coder_utils.PointBinResidual6DCoder):
            # for yaw
            point_angle_cls_labels = \
                point_reg_labels[:, 6:6 + self.box_coder.angle_bin_num]
            point_loss_angle_cls = F.cross_entropy(  # angle bin cls
            ...
            point_angle_reg_preds = (point_angle_reg_preds * point_angle_cls_labels).sum(dim=-1, keepdim=True)
            point_angle_reg_labels = (point_angle_reg_labels * point_angle_cls_labels).sum(dim=-1, keepdim=True)
            point_loss_angle_reg = self.reg_loss_func(point_angle_reg_preds[None, ...],
                                                      point_angle_reg_labels[None, ...],
                                                      weights=reg_weights[None, ...]
                                                      )
            point_loss_angle_reg = point_loss_angle_reg.squeeze()
            ...
            ###############################################################################
            # for pitch
            pitch_reg_weights = reg_weights
            point_loss_box_rxry = 0
            if self.box_coder.ground_aware:
                point_pitch_cls_labels = point_reg_labels[:, 6 + self.box_coder.angle_bin_num * 2:
                                                             6 + self.box_coder.angle_bin_num * 2 + 1]

                point_loss_pitch_cls = self.cls_pitch_loss_func(
                    point_reg_preds[:, 6 + self.box_coder.angle_bin_num * 2:
                                       6 + self.box_coder.angle_bin_num * 2 + 1],
                    point_pitch_cls_labels, weights=reg_weights).squeeze()

                pitch_reg_weights = (point_pitch_cls_labels > 0).float().view(-1)
                point_loss_pitch_cls *= loss_weights_dict['point_pitch_cls_weight']
                point_loss_box_rxry = point_loss_pitch_cls

                point_pitch_reg_preds = point_reg_preds[:, 6 + self.box_coder.angle_bin_num * 2 + 1:
                                                           6 + self.box_coder.angle_bin_num * 2 + 2]
                point_pitch_reg_labels = point_reg_labels[:, 6 + self.box_coder.angle_bin_num * 2 + 1:
                                                             6 + self.box_coder.angle_bin_num * 2 + 2]
            else:
                point_pitch_reg_preds = point_reg_preds[:, 6 + self.box_coder.angle_bin_num * 2 + 0:
                                                           6 + self.box_coder.angle_bin_num * 2 + 1]
                point_pitch_reg_labels = point_reg_labels[:, 6 + self.box_coder.angle_bin_num * 2 + 0:
                                                             6 + self.box_coder.angle_bin_num * 2 + 1]
            point_loss_pitch_reg = self.reg_loss_func(
                point_pitch_reg_preds[None, ...],
                point_pitch_reg_labels[None, ...],
                weights=pitch_reg_weights[None, ...]
            )
            point_loss_pitch_reg = point_loss_pitch_reg.squeeze() * torch.clamp(reg_weights.sum(), min=1.0) \
                                   / torch.clamp(pitch_reg_weights.sum(), min=1.0)

            point_loss_pitch_reg *= loss_weights_dict['point_pitch_reg_weight']
            point_loss_box_rxry = point_loss_box_rxry + point_loss_pitch_reg

            ###############################################################################
            # (N)
            point_loss_box = point_loss_offset_reg + point_loss_angle_cls + point_loss_angle_reg + point_loss_box_rxry

            # print(f"pitch_cls:{point_loss_pitch_cls.sum().item()}, "
            #       f"pitch_reg:{point_loss_pitch_reg.sum().item()}, "
            #       f"fg_pts:{reg_weights.sum().item()}")
        else:
    ...
    def forward(self, batch_dict):
        ...
        Args:
            batch_dict:
                ...
                gt_boxes (optional): (B, M, 8)
                point_coords_list: list{((Ni, 4))}, sasa
                point_scores_list: list{((Ni, 1))}, sasa
        Returns:
            batch_dict:
        ...
        point_coords = point_coords.view(batch_size, -1, 3).contiguous()  # point_coords(B,N,3,)
        point_features = point_features.reshape(  # point_featres(B,C,N,)
            batch_size,
            point_coords.size(1),
            -1
        ...
        # for video visualiaztion and should be deleted later.
        batch_dict['vote_offsets'] = vote_offsets

        # assign targets for vote loss
        if self.training:
            extra_width = self.model_cfg.TARGET_CONFIG.get('VOTE_EXTRA_WIDTH', None)
            targets_dict = self.assign_targets_simple(points=batch_dict['point_candidate_coords'],
        ...
        # aggregation point_feature of vote points from last SA layer xyz of backbone3d
        _, point_features, _ = self.SA_module(xyz=point_coords, features=point_features, new_xyz=vote_coords)

        #            SA              Shared-FC          CLS_FC
        # vote_coords -> point_features -> point_feature -> cls_pred
        #                               |               REG_FC
        #                               -> point_feature -> reg_pred
        ...
        point_cls_preds = point_cls_preds.view(-1, point_cls_preds.shape[-1]).contiguous()  # (B*N,C)
        point_reg_preds = point_reg_preds.permute(0, 2, 1).contiguous()
        point_reg_preds = point_reg_preds.view(-1, point_reg_preds.shape[-1]).contiguous()  # (B*N,C)

        point_cls_scores = torch.sigmoid(point_cls_preds)
        batch_dict['point_cls_scores'] = point_cls_scores

        point_box_preds = self.box_coder.decode_torch(point_reg_preds, ret_dict['point_vote_coords'])
        ...
        if self.training:
            ...
            if self.enable_sasa:
                ...
                ret_dict.update({
                    ...
                    'point_sasa_labels': point_sasa_labels  # list[(N, 1)]: assigned segmentation labels
                })
        ...
        if not self.training or self.predict_boxes_when_training:
            ...
            # point_cls_preds[:, 6:9] = 0
            batch_dict['batch_cls_preds'] = point_cls_preds
            batch_dict['batch_box_preds'] = point_box_preds
            batch_dict['cls_preds_normalized'] = False

            # TODO: for visualization only
            batch_dict['point_reg_preds'] = point_reg_preds
