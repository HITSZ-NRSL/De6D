...
__global__ void ball_query_cnt_kernel_fast(int b, int n, int m, float radius, int nsample,
    const float *__restrict__ new_xyz, const float *__restrict__ xyz, int *__restrict__ idx_cnt, int *__restrict__ idx) {
    // new_xyz: (B, M, 3)
    // xyz: (B, N, 3)
    // output:
...
void ball_query_cnt_kernel_launcher_fast(int b, int n, int m, float radius, int nsample, \
    ...
    ball_query_cnt_kernel_fast<<<blocks, threads>>>(b, n, m, radius, nsample, new_xyz, xyz, idx_cnt, idx);
