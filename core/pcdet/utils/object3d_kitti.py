...
########################################################
def get_objects3d_from_label(label_file):
    with open(label_file, 'r') as f:
        lines = f.readlines()
    objects = [ObjectPose3d(line) for line in lines]
    return objects
...
class ObjectPose3d(object):
    def __init__(self, line):
        ...
        self.loc = np.array((float(label[11]), float(label[12]), float(label[13])), dtype=np.float32)
        self.dis_to_cam = np.linalg.norm(self.loc)
        self.ry = float(label[14])  # yaw
        self.rx = float(label[15])  # pitch
        self.rz = float(label[16])  # roll
    ...
    def to_str(self):
        print_str = '%s %.3f %.3f %.3f box2d: %s hwl: [%.3f %.3f %.3f] pos: %s ry: %.3f rz: %.3f rz: %.3f' \
                    % (self.cls_type, self.truncation, self.occlusion, self.alpha, self.box2d, self.h, self.w, self.l,
                       self.loc, self.ry, self.rx, self.rz)
        return print_str
