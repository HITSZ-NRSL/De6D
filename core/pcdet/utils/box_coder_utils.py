...
##########################################################
class PointResidualCoder6D(object):
    ...
    def decode_torch(self, box_encodings, points, pred_classes=None):
        ...
        ryg = torch.atan2(sinyt, cosyt)
        rpg = torch.atan2(sinpt, cospt)
        rrg = torch.atan2(sinrt, cosrt)

        cgs = [t for t in cts]
        return torch.cat([xg, yg, zg, dxg, dyg, dzg, ryg, rpg, rrg, *cgs], dim=-1)
...
class PointBinResidualCoder(object):
    ...
    def encode_angle_torch(self, angle):
        ...
        """
        angle = torch.remainder(angle, np.pi * 2.0)  # 取余数
        angle_per_class = np.pi * 2.0 / float(self.angle_bin_num)  # 2pi/n
        # angle = ( kδ + rδ ) + 0.5δ
        shifted_angle = torch.remainder(angle + angle_per_class / 2.0, np.pi * 2.0)
        # angle = floor(( ( kδ + rδ ) + 0.5δ ) / δ ) = k + floor(r+0.5) % angle_bin_num
...
class PointBinResidual6DCoder(object):
    def __init__(self, use_mean_size=True, ground_aware=True, angle_bin_num=12,
                 minus=False, threshold=10, factor=45, **kwargs):
        super().__init__()
        # 6(xyzdxdydz) + 24(angle-cls/reg) + 2(pitch-cls/reg)
        self.ground_aware = ground_aware
        self.angle_bin_num = angle_bin_num
        self.use_mean_size = use_mean_size
        self.minus = minus
        if self.use_mean_size:
        ...
        self.code_size = 6 + 2 * self.angle_bin_num
        if self.ground_aware:
            self.code_size = self.code_size + 1 * 2
        else:
            self.code_size = self.code_size + 1
        self.ground_threshold = np.deg2rad(threshold)
        self.ground_factor = np.deg2rad(factor)

    def encode_rz_torch(self, angle):
        """
        Args:
            angle: (N)
        Returns:
    ...
    def decode_rz_torch(self, angle_cls, angle_res):
        """
        Args:
            angle_cls: (N, angle_bin_num)
            angle_res: (N, angle_bin_num)
    ...
    def encoder_rxry_torch(self, rx, ry):
        pitch, roll = ry, rx
        if self.ground_aware:
            if self.minus:
                angle_cls = torch.abs(pitch) > self.ground_threshold
                angle_res = torch.zeros_like(pitch)
                angle_res[angle_cls] = pitch[angle_cls] / self.ground_factor
                # angle_res[torch.logical_not(angle_cls)] = 0
            else:
                angle_cls = pitch < -self.ground_threshold
                angle_res = torch.zeros_like(pitch)
                angle_res[angle_cls] = (-self.ground_threshold - pitch[angle_cls]) / self.ground_factor
            return angle_cls.long(), angle_res
        else:
            angle_res = pitch
            return angle_res,

    def decoder_rxrz_torch(self, rxrz_code):
        if self.ground_aware:
            pitch_cls = rxrz_code[:, 0]
            pitch_res = rxrz_code[:, 1]
            no_pitch_flag = torch.sigmoid(pitch_cls) < 0.5
            if self.minus:
                pitch = pitch_res * self.ground_factor
            else:
                pitch = -self.ground_threshold - pitch_res * self.ground_factor
            pitch[no_pitch_flag] = 0
            roll = torch.zeros_like(pitch)
            return pitch.view(-1, 1), roll.view(-1, 1)
        else:
            pitch = rxrz_code[:, 0]
            roll = torch.zeros_like(pitch)
            return pitch.view(-1, 1), roll.view(-1, 1)

    def decode_torch_kernel(self, box_offsets, box_rots, points, pred_classes=None):
        """
        Args:
            box_offsets: (N, 6) [x, y, z, dx, dy, dz]
            box_angle_cls: (N, angle_bin_num)
        ...
        box_rz_cls = box_rots[:, :self.angle_bin_num]
        box_rz_reg = box_rots[:, self.angle_bin_num:self.angle_bin_num * 2]
        box_rxrz_code = box_rots[:, self.angle_bin_num * 2:]

        rzg = self.decode_rz_torch(box_rz_cls, box_rz_reg).unsqueeze(-1)
        rxrzg = self.decoder_rxrz_torch(box_rxrz_code)
        return torch.cat([xg, yg, zg, dxg, dyg, dzg, rzg, *rxrzg], dim=-1)
    ...
    def encode_torch(self, gt_boxes, points, gt_classes=None):
        ...
        Returns:
            box_coding: (N, 6 + 2 * B + C)
        """
        assert gt_boxes.shape[-1] >= 6 + 3, 'gt_boxes shape: %s' % str(gt_boxes.shape)
        gt_boxes[:, 3:6] = torch.clamp_min(gt_boxes[:, 3:6], min=1e-5)  # avoid log overflow

        xg, yg, zg, dxg, dyg, dzg, rzg, ryg, rxg, *cgs = torch.split(gt_boxes, 1, dim=-1)
        xa, ya, za = torch.split(points, 1, dim=-1)
        ...
        # for rotation
        rzg_cls, rzg_reg = self.encode_rz_torch(rzg.squeeze(-1))
        rxry_code = self.encoder_rxry_torch(rxg, ryg)
        return torch.cat([xt, yt, zt, dxt, dyt, dzt,
                          rzg_cls, rzg_reg, *rxry_code], dim=-1)
    ...
    def decode_torch(self, box_encodings, points, pred_classes=None):
        ...
        Returns:
            boxes3d: (N, 7)
        """
        box_offsets, box_rot_code, *cgs = \
            torch.split(box_encodings, [6, self.angle_bin_num * 2 + (2 if self.ground_aware else 1)], dim=-1)

        boxes3d = self.decode_torch_kernel(box_offsets, box_rot_code, points, pred_classes)
